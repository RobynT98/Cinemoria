...
export default function MovieForm({ initial, submitLabel, onSubmit }: MovieFormProps) {
  ...
  async function fetchFromOMDb() {
    ...
    try {
      ...
      // mappa fält
      const poster = typeof data.Poster === "string" && data.Poster !== "N/A" ? data.Poster : "";
    ...
    <>
      <div className="card p-4 space-y-3">
        ...
        <div className="flex gap-2 flex-wrap">
          <button className="btn" onClick={() => setShowScan(true)}>Skanna streckkod</button>
          <button className="btn" onClick={fetchFromOMDb}>Hämta från OMDb</button>
        </div>
