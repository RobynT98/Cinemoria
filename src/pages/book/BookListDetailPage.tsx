...
export default function BookListDetailPage() {
  ...
  const [list, setList] = useState<BookList | undefined>();
        ...
        {inList.length === 0 ? (
          ...
          <div className="mt-2 space-y-2">
            {inList
              ...
              .map((b) => (
                <div key={b.id} className="flex items-center justify-between gap-2 border-b border-ink-700/30 pb-2">
                  <div className="min-w-0">
                    <div className="font-medium truncate">{b.title}</div>
                    <div className="text-sand-300 text-xs">
                      {(b.author || "Okänd")} {b.year ? `• ${b.year}` : ""}
                      {b.format ? ` • ${labelBookFormat(b.format)}` : ""}
                    </div>
                  </div>
                  <button className="chip" onClick={() => handleRemove(b)} disabled={busy} title="Ta bort från listan">
                    <X size={14} /> Ta bort
                  </button>
                </div>
              ))}
